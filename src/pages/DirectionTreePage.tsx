  ...
  return (
    <div className="mx-auto max-w-5xl px-5 pb-24 pt-24 sm:px-8">
      ...
      <motion.div
        ...
        className="mb-8 flex flex-wrap items-center gap-x-5 gap-y-2 rounded-lg border border-border bg-card px-4 py-3"
      >
        <span className="text-[11px] uppercase tracking-wider text-muted-foreground/60">接触程度</span>
      ...
      {directions.length === 0 ? (
        ...
        >
          <Compass className="mx-auto h-10 w-10 text-muted-foreground/40" strokeWidth={1.25} />
          <h3 className="font-serif-cn mt-4 text-xl text-muted-foreground">尚未识别认知方向</h3>
          <p className="mt-2 text-sm text-muted-foreground/70">完成诊断对话以生成你的方向树</p>
        </motion.div>
      ) : (
            ...
            return (
              ...
              >
                <div className="mb-3 flex items-center gap-2">
                  ...
                  <h2 className="font-serif-cn text-lg font-medium text-foreground">{dir.name}</h2>
                  <span className="text-[11px] text-muted-foreground/60">认知大方向</span>
                  <span className="ml-auto rounded-full border border-border bg-card px-2 py-0.5 text-[11px] text-muted-foreground/80">
                    {dirTouched} / {dirTotal} 子领域
                  </span>
                </div>

                {dir.subfields.length === 0 ? (
                  <div className="rounded-lg border border-dashed border-border bg-card/50 py-6 text-center text-[12px] text-muted-foreground/50">
                    ...
                    {dir.subfields.map((sub, si) => {
                      ...
                      return (
                        <motion.button
                          ...
                          className={cn(
                            "group flex items-center gap-3 rounded-lg border p-3 text-left transition-all duration-200 hover:border-primary/30",
                            meta.rowClass,
                            isSel ? "ring-2 ring-primary/30" : "ring-0"
                          )}
                        >
                          {/* 状态图标 */}
                          {sub.exposure === "high" ? (
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="truncate text-[13px] font-medium text-foreground">{sub.name}</div>
          ...
          >
            ...
            >
              <div className="flex items-center justify-between">
                <h3 className="font-serif-cn text-lg font-medium text-foreground">添加新大方向</h3>
                <button onClick={() => setShowAddDirection(false)} className="text-muted-foreground/70 hover:text-muted-foreground">
                  <X className="h-4 w-4" />
                </button>
              </div>
              ...
              <input
                ...
                placeholder="如：机器学习"
                autoFocus
                className="mt-4 w-full rounded-lg border border-border bg-card px-3 py-2.5 text-sm text-foreground outline-none transition-colors placeholder:text-muted-foreground/50 focus:border-primary/50"
              />
              <div className="mt-4 flex justify-end gap-2">
                <button
                  onClick={() => { setShowAddDirection(false); setNewDirectionName(""); }}
                  className="rounded-lg px-4 py-2 text-[13px] text-muted-foreground transition-colors hover:text-foreground/85"
                >
                  取消
                </button>
                <button
                  ...
                  className="rounded-lg bg-muted px-4 py-2 text-[13px] font-medium text-foreground transition-colors hover:bg-muted/80 disabled:opacity-40"
                >
                  添加
                </button>
              </div>
      ...
      {selected && (
        ...
        >
          <div className="flex items-center justify-between">
            <div>
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground/60">已选中</div>
              <div className="font-serif-cn mt-0.5 text-lg font-medium text-foreground">{selected.subfield.name}</div>
              <div className="mt-0.5 text-[11px] text-muted-foreground/70">方向 · {selected.direction.name}</div>
            </div>
            <div className="text-right">
              <div className="text-[11px] uppercase tracking-wider text-muted-foreground/60">接触程度</div>
...
function StatBox({ label, value, accent }: { label: string; value: number; accent: string }) {
  return (
    <div className="rounded-lg border border-border bg-card p-4 text-center">
      ...
      <div className="mt-1 text-[11px] uppercase tracking-wider text-muted-foreground/60">{label}</div>
    </div>
  );
}
